...
function modelClass(MongooseModel) {
  class Model extends MongooseModel {
    ...
    /**
     * @param ids {String[]} 要求 ids 没有重复
     * @param fields {Object|Array}
     * @param options {Object}
     * @return {Promise<Model[]>}
     */
    static async findAllByIds(ids, fields, options) {
      const models = await this.findAll({ _id: { $in: ids } }, fields, options);

      if (models.length !== ids.length) {
        throw new Error(`findAllByIds "${this.modelName}" failed`);
      }
      return models;
    }
    ...
    static async findOne(conditions, fields, options) {
      ...
      if (model === null) {
        throw new Error(`findOne "${this.modelName}" failed`);
      }
      return model;
    }
