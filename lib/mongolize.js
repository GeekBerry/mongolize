...
const subClass = require('./sub_class');
...
class Mongolize extends mongoose.Connection {
  ...
  define(name, schemaObject, options) {
    ...
    const MongooseModel = this.model(name, schema, name); // 设置 model 和 collection 同名
    return subClass(MongooseModel);
  }
}

module.export = Mongolize;
